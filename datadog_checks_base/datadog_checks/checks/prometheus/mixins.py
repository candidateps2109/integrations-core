...
class PrometheusScraperMixin(object):
    ...
    def create_mixin_configuration(self, instance, default_namespace=None, default_instances=None):
        if default_instances is None:
            default_instance = {}

        # Create an empty configu
        config = {}
        ...
        default_instance = default_instances.get(namespace, {})
        ...
        config['send_histograms_buckets'] = instance.get('send_histograms_buckets', default_instance.get('send_histograms_buckets', True))
        ...
        config['send_monotonic_counter'] = instance.get('send_monotonic_counter', default_instance.get('send_monotonic_counter', True))
        ...
        config['health_service_check'] = instance.get('health_service_check', default_instance.get('health_service_check', True))
    ...
    def _parse_metric_family(self, response, scraper_config):
        """
    ...
    def scrape_metrics(self, endpoint, scraper_config):
        """
        Poll the data from prometheus and return the metrics as a generator.
        """
        response = self.poll(endpoint, scraper_config)
        try:
    ...
    def process(self, endpoint, scraper_config, ignore_unmapped=False):
        """
    ...
    def process_metric(self, message, scraper_config, ignore_unmapped=False):
        """
            ...
            if not ignore_unmapped:
                if message.name in scraper_config['metric_transformer']:
                    try:
                        ...
                        transformer = scraper_config['metric_handler'][message.name]
                        transformer(message)
                    except Exception as err:
                        self.log.warning("Error handling metric: {} - error: {}".format(message.name, err))
                else:
                    self.log.debug("Unable to handle metric: {} - error: No handler function defined".format(message.name))
            else:
